...
export class WACZ {
   ...
   */
  async finalize (sign = false, autoindex = true) {
    ...
    this.files['datapackage-digest.json'] = await this.generateDatapackageDigest(sign)
 ...
 * @param {string} token - Optional token to be passed in the Authorization header
 * @return {Promise} a promise of https.request
 */
function requestSignature(url, payload, auth) {
  return new Promise((resolve, reject) => {
    ...
    req.on('timeout', () => {
      req.destroy()
      reject(new Error(`Request to WACZ signing server timed out. URL: ${url}`))
    })
